...
type Options = DefaultOptions & {
  FactoryClassCode: boolean | undefined;
  CollapseSchema: boolean | undefined;
  MandatoryStructName: boolean | undefined;
};
  ...
  static ${className} Function(dynamic data, List<String> path) parse() {
    return ${className}.fromJson;
      ...
      return {
        ...
        parser:
          `NullableClass.parse<${nonNullableSchema.type}>(${nonNullableSchema.parser})`,
      };
    }
...
class ${className} extends BaseListClass<${itemSchemaCode.type}> {
  ...
  static ${className} Function(dynamic data, List<String> path) parse() {
    ...
    return {
      ...
      parser:
        `BaseListClass.parse<${itemSchemaCode.type}>(${itemSchemaCode.parser})`,
    };
  }
...
class ${className} extends BaseDiscriminatorClass<${className}Mixin> {
  ...
  ${getters.join("\n  ")}
    ...
    return {
      ...
      parser: `BaseDiscriminatorClass.parse<BaseStructClass>(${param}, {${
        mapping.join(
          ",",
        )
      }})`,
...
class ${className} extends BaseMapClass<${keySchemaCode.type}, ${valueSchemaCode.type}> {
  ...
  static ${className} Function(dynamic data, List<String> path) parse() {
    ...
    return {
      parser:
        `BaseMapClass.parse<${keySchemaCode.type}, ${valueSchemaCode.type}>(${keySchemaCode.parser}, ${valueSchemaCode.parser})`,
  ...
  if (schema.type === "object" || schema.allOf) {
    ...
    for (const propName in schema.properties) {
      ...
      let isRequired = Object.keys(valueSchema).length &&
...
class ${className} extends BaseStructClass with ${mixins.join(",")} {
  ...
  static ${className} Function(dynamic data, List<String> path) parse() {
    ...
    return {
      parser: `BaseStructClass.parse({${
        factories
          .map((x) => `...${x}`)
          .join(
            ",",
          )
      }}, const {${required.map((x) => `...${x}`).join(",")}}, null)`,
  ...
  if (schema.enum || schema.const) {
    ...
    for (const s of constants) {
      if (
        typeof s === "string" &&
        /^[a-zA-Z_$][a-zA-Z_$0-9]*$/.test(s) &&
        !enums.has(s)
      ) {
        initializers.push(
          `static const literal${s[0].toUpperCase()}${
            s.slice(1)
          } = ${enumName}._(${JSON.stringify(s)});`,
        );
        enums.add(s);
      }
...
class ${tag} {
  ...
  const parameters = route.parameters?.map(function (x) {
  ...
  const responseHeaders = resHeaders.length
    ...
    : {
      ...
      parser:
        "((dynamic headers, List<String> path) => const BaseLiteralClass._(null))",
    };
  const responseBody = "$ref" in resBody
...
class Request${name} extends RequestClass<${types.join(", ")}> {
  Request${name}(${!args.length ? "" : `{${args.join(", ")}}`}): super(${
    nullArgs.join(", ")
  });
}
class Response${name} extends ResponseClass<${responseHeaders.type}, ${responseBody.type}> {
...
class Api${name} extends BaseApiClass<Request${name}, Response${name}> {
  Api${name}.build(super.request): super(path: ${
    JSON.stringify(
      path,
    )
 ...
 * Generate Dart code
 */
export function genDartCode(
  input: { json: OpenAPIObject; options: Options },
): Options {
  json = input.json;
